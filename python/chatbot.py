...
class ChatbotTeam:
    def __init__(self):
        ...
        for url in urls:
            try:
                AI_PLUGINS.append(AIPlugin.from_url(
        ...
        vector_store = vectorizer.fit_transform(
        ...
        self.tools = self.get_tools(
            "Generate 10 ideas for making money trading stocks or performing other online services or tasks.")
    ...
    def get_tools(self, query):
        ...
        similarities = cosine_similarity(
            query_vector, self.retriever).flatten()
        sorted_indices = similarities.argsort()[::-1]
        relevant_docs = [docs[idx]
                         for idx in sorted_indices if similarities[idx] > 0]
        tool_kits = [toolkits_dict[doc.metadata["plugin_name"]]
